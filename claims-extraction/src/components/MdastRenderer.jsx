...
function Node({ node, references, onFigureClick, onTableClick, onReferenceClick }) {
  ...
  switch (node.type) {
    ...
    case 'image': {
      const src = FIGURE_IMAGE_OVERRIDES[node.enumerator] || node.url
...
const FIGURE_IMAGE_OVERRIDES = {
  ...
  4: `${BASE}assets/628638v2_fig4.webp`,
  5: `${BASE}assets/628638v2_fig5.webp`,
}
...
function FigureCard({ node, references, onFigureClick, onReferenceClick }) {
  ...
  const defaultSrc = FIGURE_IMAGE_OVERRIDES[node.enumerator] || imageNode?.url
